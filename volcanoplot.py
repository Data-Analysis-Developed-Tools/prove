...
def color_scale(value, vmin, vmax):
    ...
    if pd.isna(value):
        return "background-color: #f1f1f1"  # Grigio per NaN
    else:
...
def display_data_with_style(df):
    vmin = df['-log10(p-value) x Log2FoldChange'].min()
    vmax = df['-log10(p-value) x Log2FoldChange'].max()
    return df.style.applymap(lambda x: color_scale(x, vmin, vmax), subset=['-log10(p-value) x Log2FoldChange'])
    ...
    if file is not None:
        ...
        if dati is not None:
            ...
            if dati_preparati is not None:
                ...
                st.plotly_chart(fig)
                # Visualizza i dati sotto il grafico in forma di tabella con stili condizionali
