...
import numpy as np
import plotly.graph_objects as go  # Per elementi grafici aggiuntivi
        ...
        for var in dati.index:
            ...
            if len(valori[0]) > 0 and len(valori[1]) > 0:
                ...
                if abs(media_diff) >= fold_change_threshold and p_val_log >= p_value_threshold:
                    risultati.append([var, media_diff, p_val_log, np.mean(np.hstack(valori))])  # Aggiunge la media per la colorazione
...
# Crea il volcano plot con linee, annotazioni e controllo interattivo
def crea_volcano_plot(dati, classi, show_labels, size_scale, color_scale):
    if dati is not None:
        size = np.log10(dati['Media'] + 1) * size_scale  # Scala per il diametro dei pallini
        if color_scale:
            color = np.log10(dati['Media'] + 1)
        else:
            color = None
        ...
        fig = px.scatter(dati, x='Log2FoldChange', y='-log10(p-value)', text='Variabile' if show_labels else None,
                         ...
                         color_continuous_scale='Viridis')  # Scala di colore da blu a rosso

        fig.add_trace(go.Scatter(x=[0, 0], y=[0, dati['-log10(p-value)'].max()], mode='lines',
                                 line=dict(color='orange', width=2)))

        fig.add_annotation(x=-2, y=dati['-log10(p-value)'].max()*1.05, text=f"Over-expression in {classi[1]}",
                           showarrow=False, font=dict(color="red", size=16))
        fig.add_annotation(x=2, y=dati['-log10(p-value)'].max()*1.05, text=f"Over-expression in {classi[0]}",
...
def main():
    ...
    with st.form(key='my_form'):
        fold_change_threshold = st.number_input('Inserisci il valore soglia per il Log2FoldChange', value=0.00)
        ...
        size_scale = st.checkbox("Aggiusta dimensione dei pallini", value=True)
        color_scale = st.checkbox("Colora i pallini secondo la media", value=True)
    ...
    if file is not None and submit_button:
        ...
        if dati is not None:
            ...
            if dati_preparati is not None:
                fig = crea_volcano_plot(dati_preparati, classi, show_labels, 2*size_scale, color_scale)  # Doppia la scala per il diametro
                if fig is not None:
                    st.plotly_chart(fig)
                else:
