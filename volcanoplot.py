            ...
            if dati_preparati is not None:
                ...
                if fig is not None:
                    ...
                    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                        dati_preparati.to_excel(writer, index=False, sheet_name='Filtered Data')
                        writer.save()
                    output.seek(0)
                    st.download_button(label="Scarica i dati filtrati come Excel", data=output, file_name="filtered_data.xlsx", mime="application/vnd.ms-excel")
